...
from .geomutils import (to_2D, get_transform_func,
                        geom_type_mapping, unchanged_geom)
...
class PostGISGeomRefDB(GeomRefDB):
    ...
    def get_PG_geoms_EPSG(self):
        ...
        PG_cursor.execute(f"SELECT Find_SRID('{self.PG_schema}', "
                          f"'{self.PG_table}', '{self.PG_geoms_column}')")
    ...
    def true_positives(self, geoms_iter, geoms_EPSG, same_geoms_func):
        ...
        if transform:
	        SQL_query_template = (f"SELECT ST_AsBinary(ST_Transform({self.PG_geoms_column},"
	                              f"{geoms_EPSG})) "
                                  f"FROM {path2table} "
	        ...
	        SQL_query_template = (f"SELECT ST_AsBinary({self.PG_geoms_column}) "
                                  f"FROM {path2table} "
                                  f"WHERE ST_Intersects({self.PG_geoms_column}, "
    ...
    def missing_geometries(self, geoms_iter, AOI_geom, geoms_EPSG, same_geoms_func):
        ...
        if transform:
            ...
            project = pyproj.Transformer.from_crs(AOI_crs, PG_crs, always_xy=True).transform
            aoi_wkt = shapely.ops.transform(project, AOI_geom).wkt
            SQL_query = (f"SELECT ST_AsBinary(ST_Transform({self.PG_geoms_column},"
                         f"{geoms_EPSG})) "
                         f"FROM {path2table} "
                         f"WHERE ST_Intersects({self.PG_geoms_column}, "
                         f"ST_GeomFromText('{aoi_wkt}', {PG_geoms_EPSG}))")
        else:
	        SQL_query = (f"SELECT ST_AsBinary({self.PG_geoms_column}) "
                         f"FROM {path2table} "
                         f"WHERE ST_Intersects({self.PG_geoms_column}, "
        ...
        for row in PG_cursor:
            PG_geom = wkb.loads(row[0].tobytes())
            if not any(same_geoms_func(el.object, PG_geom)
                       for el in index.intersection(PG_geom.bounds,
                                                    objects=True)):
                yield PG_geom
        PG_cursor = None
...
class RtreeGeomRefDB(GeomRefDB):
    ...
    def true_positives(self, geoms_iter, geoms_EPSG, same_geoms_func):
        ...
        if transform:
            ...
            project = pyproj.Transformer.from_crs(geoms_crs, ref_DB_crs,
                                                  always_xy=True).transform
            for geom in geoms_iter:
                geom_reproj = shapely.ops.transform(project, geom)
                if any(same_geoms_func(geom_reproj, el.object)
                       for el in self.index.intersection(geom_reproj.bounds,
                                                         objects=True)):
                    yield geom
        else:
            for geom in geoms_iter:
                if any(same_geoms_func(geom, el.object)
                       for el in self.index.intersection(geom.bounds,
                                                         objects=True)):
                    yield geom            
        logger.info("Done searching true positive geometries.")
    ...
    def false_positives(self, geoms_iter, geoms_EPSG, same_geoms_func):
        ...
        if transform:
            geoms_crs = pyproj.CRS(f"EPSG:{geoms_EPSG}")
            ref_DB_crs = pyproj.CRS(f"EPSG:{self.EPSG}")
            project = pyproj.Transformer.from_crs(geoms_crs, ref_DB_crs_crs,
                                                  always_xy=True).transform        
            for geom in geoms_iter:
                geom_reproj = shapely.ops.transform(project, geom)
                if not any(same_geoms_func(geom_reproj, el.object)
                           for el in self.index.intersection(geom_reproj.bounds,
                                                             objects=True)):
                    yield geom
        else:
            for geom in geoms_iter:
                if not any(same_geoms_func(geom, el.object)
                           for el in self.index.intersection(geom.bounds,
                                                             objects=True)):
                    yield geom
        logger.info("Done searching false positive geometries.")
    ...
    def missing_geometries(self, geoms_iter, AOI_geom, geoms_EPSG, same_geoms_func):
        ...
        if transform:
            geoms_crs = pyproj.CRS(f"EPSG:{geoms_EPSG}")
            ref_DB_crs = pyproj.CRS(f"EPSG:{self.EPSG}")
            project = pyproj.Transformer.from_crs(ref_DB_crs, geoms_crs,
                                                  always_xy=True).transform
            if AOI_geom is not None:
                project_AOI = pyproj.Transformer.from_crs(geoms_crs, ref_DB_crs,
                                                          always_xy=True).transform
            ...
            for ref_geom in self.intersecting_idx_geoms(poly=AOI_geom):
                ...
                if not any(same_geoms_func(el.object, ref_geom)
                           for el in index.intersection(ref_geom.bounds,
                                                        objects=True)):
                    yield ref_geom
        else:
            for ref_geom in self.intersecting_idx_geoms(poly=AOI_geom):
...
class SQLiteGeomRefDB(GeomRefDB):
    ...
    def __init__(self, db_path=None, default_epsg=None, geoms_iter=None,
                 geoms_tab_name=None, geom_type=None, geoms_epsg=None,
                 in_ram=True, logger=None, logger_name=None,
        ...
        if default_epsg is not None:
            try:
                ...
                raise ValueError("{!r} ('default_epsg') is not a valid "
                                 "EPSG code!")
            else:
        ...
        if db_path is None:
            if not in_ram:
                raise ValueError("The 'db_path' and 'in_ram' parameters "
                                 "cannot both be None!")
            else:
            ...
            if not new_db:
                ...
                disk_conn.close()
                self.logger.info(f"Database file {db_path!r} successfully "
                                 "loaded in RAM.")
            else:
                cursor = conn.cursor()
                cursor.execute("SELECT InitSpatialMetaData();")
                self.conn.commit()
                self.logger.info(f"File {dp_path!r} does not exist, new database "
                                 "created in RAM.")
        else:
        ...
        if geoms_iter is not None:
            try:
                self.add_geometries(geoms_iter, geom_type=geom_type,
                                    geoms_epsg=geoms_epsg,
                                    geoms_tab_name=geoms_tab_name)
            except Exception:
                if new_db and not in_ram:
                    self.logger.error("An error occurred while adding "
                                      "geometries to the database, deleting "
                                      f"file {db_path!r}...")
                    os.remove(db_path)
                else:
                    self.logger.error("An error occurred while adding "
                                      "geometries to the database...")
                raise
    ...
    def save_db(self, path, overwrite=True):
        if os.path.isfile(path):
            if overwrite:
                self.logger.info(f"File {path!r} already exists. Removing "
                                 "file...")
                os.remove(path)
            else:
                self.logger.info(f"File {path!r} already exists. Saving "
                                 "database aborted.")
                return
    ...
    def add_geometries(self, geoms_iter, geom_type=None, geoms_epsg=None,
                       geoms_tab_name=None):
        ...
        if geom_type is not None and not geom_type in self.supported_geom_types:
            raise ValueError(f"{geom_type!r} is not a valid value for the "
                             "'geom_type' argument! Supported geometry "
                             "types are: {}."
                             .format(", ".join([f"{gt!r}"
                                                for gt in self.supported_geom_types])))
        ...
        if new_tab:
            if geom_type is None:
                raise ValueError("'geom_type' cannot be passed None for "
                                 "new databases, or if no existing geometry "
                                 "table name ('geoms_tab_name') was passed "
                                 "as parameter!")
            ...
            if geom_type is not None and geom_type != tab_info["geom_type"]:
                raise ValueError(f"The {geom_type!r} geometry type does not match "
                                 f"the geometry type of the {geoms_tab_name!r} table!")
            if geoms_epsg is None:
                ...
                geoms_epsg = tab_info["srid"]
        for geom in geoms_iter:
            cursor.execute(f"INSERT INTO {geoms_tab_name} (geometry) VALUES "
    ...
    def db_geom_info(self, to_stdout=False):
        ...
        info = {tab[0]: {"geom_type": geom_type_mapping[tab[2]], "srid": tab[4]}
        ...
        if not to_stdout:
            ...
            f1 = "TABLE"
            f1_width = max(max(len(tab_name) for tab_name in info.keys()),
                           len(f1))
            f2 = "GEOMETRY"
            f2_width = max(max(len(info[k]["geom_type"]) for k in info.keys()),
                           len(f2))
            f3 = "EPSG"
            f3_width = max(max(len(str(info[k]["srid"])) for k in info.keys()),
                           len(f3))
    ...
    @staticmethod
    def _get_spatial_query(spatial_index=True, only_within_aoi=False,
                           within_aoi=False):
    ...
    def _geoms_generator(self, geoms_iter, transform_geom,
                         geoms_match, get_search_frame, query,
                         query_kwargs, matching_geoms=True):
        cursor = self.conn.cursor()
        if matching_geoms:
            for geom in geoms_iter:
                ...
                if any(geoms_match(geom_reproj, wkb.loads(row[0]))
                       for row in cursor):
                    yield geom
        else:
            for geom in geoms_iter:
                ...
                query_kwargs["geomsfwkt"] = search_frame.wkt
                cursor.execute(query.format(**query_kwargs))            
                if not any(geoms_match(geom_reproj, wkb.loads(row[0]))
                           for row in cursor):
                    yield geom


    def true_positives(self, geoms_iter, aoi_geom=None, geoms_epsg=None,
                       geoms_tab_name=None, geoms_match=None,
                       get_search_frame=None, ncores=None):#, **kwargs):
        self.logger.info("Searching true positive geometries...")
        query_kwargs = dict()
        ...
        if ncores is not None:
            try:
                ...
                if ncores > max_cores:
                    self.logger.info(f"Value {ncores} passed to the 'ncores' "
                                     "argument is too high and may "
                                     "result in performance penalty, "
                                     f"setting it down to {max_cores}.")
                    ncores = max_cores
                elif ncores < 2:
                    ncores = None                    
            except (ValueError, TypeError):
                raise ValueError(f"{ncores!r} is not a valid value for the "
                                 "'ncores' argument!")
        query = self._get_spatial_query(within_aoi=aoi_geom is not None)
        if ncores is not None:
            tp_gen = self._parallelized_method(ncores, geoms_iter,
                                               transform_geom,
                                               geoms_match, get_search_frame,
                                               query, query_kwargs,
                                               matching_geoms=True,
                                               method_name="_geoms_generator")
        else:
            tp_gen = self._geoms_generator(geoms_iter, transform_geom,
                                           geoms_match, get_search_frame,
                                           query, query_kwargs,
                                           matching_geoms=True)
        yield from tp_gen
    ...
    def false_positives(self, geoms_iter, aoi_geom=None, geoms_epsg=None,
                        geoms_tab_name=None, geoms_match=None,
                        get_search_frame=None, ncores=None):#, **kwargs):
        ...
        query = self._get_spatial_query(within_aoi=aoi_geom is not None)        
        if ncores is not None:
            fp_gen = self._parallelized_method(ncores, geoms_iter,
                                               transform_geom,
                                               geoms_match, get_search_frame,
                                               query, query_kwargs,
            ...
            fp_gen = self._geoms_generator(geoms_iter, transform_geom,
                                           geoms_match, get_search_frame,
                                           query, query_kwargs,
                                           matching_geoms=False)
        yield from fp_gen
    ...
    def missing_geometries(self, geoms_iter, geom_type=None, aoi_geom=None,
                           geoms_epsg=None, geoms_tab_name=None,
                           geoms_match=None, get_search_frame=None,
                           ncores=None):#, **kwargs):
        ...
        if tab_info is None:
            raise RuntimeError("No {!r} table was found in the database!")
        if geom_type is None:
            self.logger.info("No geometry type was passed to the 'geom_type' "
                             "argument. Assuming that all input geometries "
                             "have the same geometry type as the ones stored "
                             f"in the {geoms_tab_name!r} table.")
        ...
        if ncores is not None:
            try:
                ...
                raise ValueError(f"{ncores!r} is not a valid value for the "
                                 "'ncores' argument!")
        if ncores is None:
            logger_conf.update({"logger_name": str(uuid.uuid1()),
                                "logging_level": None})
        else:
            logger_conf["logger"] = self.logger            

        input_geoms_db = SQLiteGeomRefDB(geoms_iter=geoms_iter,
                                         geom_type=geom_type,
                                         geoms_epsg=geoms_epsg,
                                         **logger_conf)
        ...
        if ncores is not None:
            mg_gen = input_geoms_db._parallelized_method(ncores, geoms_iter,
            ...
            mg_gen = input_geoms_db.false_positives(geoms_iter,
                                                    geoms_epsg=db_epsg,
                                                    geoms_match=geoms_match,
                                                    get_search_frame=get_search_frame)
        yield from mg_gen
    ...
    def _parallelized_method(self, ncores, geoms_iter, *method_args,
                             method_name="", **method_kwargs):
        if not method_name:
            method_name = inspect.stack()[1][3]
        method_obj = getattr(self, method_name, None)
        if method_obj is None:
            raise ValueError(f"{method_name!r} is not a valid method name for "
                             f"the class {cls.__name__!r}!")
        logger_conf = {"name": self.logger.name,
        ...
        for i in range(ncores):
            procs.append(mp.Process(target=self._wrap_method_return,
                                    args=[method_obj, geoms_iters[i],
                                          shared_results_iter, nprocs_done,
                                          *method_args],
    ...
    def _wrap_method_return(self, method_obj, geoms_iter, shared_results_iter,
                            nprocs_done, *method_args,
                            logger_conf=None, **method_kwargs): 
        if logger_conf is None:
            logger = setup_logger(name=str(uuid.uuid1()), level=None)
        else:
            logger_conf["name"] = (logger_conf.get("name", type(self).__name__)
                                   + f" (PID: {os.getpid()})")
